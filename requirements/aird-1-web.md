...
See /requirements/file-structure.md
...
- Never use SELECT * in database queries. Always explicitly specify the required fields for better performance, maintainability, and clarity. For example:
  ...
  `)
  ```
  - Don't delete comments unless they're outdated, incorrect, or explicitly asked to be removed
  - Provide complete and accurate comments for all code
  - Use `//` for single-line comments and `/* ... */` for multi-line comments
  - Don't comment the obvious - e.g. don't comment the obvious return statements, or simple assignments
  - Check for errors and edge cases in your code and handle them appropriately
  - Check that you are importing functions that exist, don't assume a file exists just because you think it might make sense
  - Ask the user if you want access to a file that you don't have in your context
