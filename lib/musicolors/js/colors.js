...
function applyTimbre() {
  // Use energy directly for size like original
  if (!energy || energy < 0.001) {
    size = MusicolorsConfig.visualization.minSize;
  } else {
    size = energy * MusicolorsConfig.visualization.sizeMultipliers.linear;
  }

  // Direct color mapping like original
  hue = warmth;
  saturation = richness * 100;
  luminance = sharpness * 100;
  ...
  // Create geometry with direct size
