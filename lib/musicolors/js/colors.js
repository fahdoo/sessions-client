...
// Default values in case config isn't loaded yet
const defaultConfig = {
  visualization: {
    defaultSize: 1,
    minSize: 1,
...
// Initialize with either config values or defaults
let currentSize = (MusicolorsConfig?.visualization?.defaultSize ?? defaultConfig.visualization.defaultSize);
let currentHue = (MusicolorsConfig?.visualization?.defaultColors?.hue ?? defaultConfig.visualization.defaultColors.hue);
let currentSaturation = (MusicolorsConfig?.visualization?.defaultColors?.saturation ?? defaultConfig.visualization.defaultColors.saturation);
let currentLuminance = (MusicolorsConfig?.visualization?.defaultColors?.luminance ?? defaultConfig.visualization.defaultColors.luminance);
const transitionSpeed = (MusicolorsConfig?.visualization?.transitionSpeed ?? defaultConfig.visualization.transitionSpeed);
...
function createVanilla(){
    // Create a much smaller initial sphere
    geometry = new THREE.SphereGeometry(
        MusicolorsConfig.visualization.minSize * 0.5, // Half of minimum size
        128,
        128
    );

    // Use a more subtle material
    ...
    compoCenter.scale.set(
        MusicolorsConfig.visualization.scale * 0.5, // Half scale
        MusicolorsConfig.visualization.scale * 0.5,
        MusicolorsConfig.visualization.scale * 0.5
    );
    ...
    pointLight = new THREE.PointLight(
        ...
        MusicolorsConfig.lighting.point.intensity * 0.5, // Reduced intensity
    ...
    console.log('Initial sphere created (minimal)');
}
...
function getColorByPitch(pitch) {
  // Use rainbow colors from config with fallback
  const rainbowColors = MusicolorsConfig?.palettes?.rainbow || {};
  const fallbackColor = MusicolorsConfig?.visualization?.defaultColors?.fallbackColor1 || '#FFFFFF';
  ...
  // Return fallback color if we get here
  return fallbackColor;
}
...
function applyTimbre() {
  // Calculate size with strict minimum enforcement
  let targetSize;
  if (!energy || energy < 0.001) {
    targetSize = MusicolorsConfig.visualization.minSize;
  } else {
    const energyScaled = energy * MusicolorsConfig.visualization.sizeMultipliers.linear;
    // Ensure size never goes below minimum
    targetSize = Math.max(
      MusicolorsConfig.visualization.minSize,
      energyScaled
    );
    // And cap at maximum
    targetSize = Math.min(targetSize, MusicolorsConfig.visualization.maxSize);
  }

  // Use the enforced size
  size = targetSize;
  ...
  // Create geometry with enforced size
  geometry = new THREE.SphereGeometry(size, 128, 128);

  // Create material
  material = new THREE.ShaderMaterial({
    uniforms: {
      color1: { value: new THREE.Color(hex1 || '#FFFFFF') },
      color2: { value: new THREE.Color(hex2 || '#000000') }
    },
    vertexShader: `
      varying vec2 vUv;
      void main() {
